"""The Dreame Smart Vacuum component."""
...
async def _async_register_card(hass: HomeAssistant) -> None:
    """Expose dreame-smart-vacuum-card.js and auto-load it on every dashboard."""
    ...
    if not card_path.is_file():
        LOGGER.warning("Dreame Smart Vacuum card not found at %s", card_path)
        return
    ...
    try:
        ...
        LOGGER.debug("Dreame Smart Vacuum card path already registered: %s", ex)
    ...
    LOGGER.debug("Dreame Smart Vacuum card registered at %s?v=%s", CARD_URL_PATH, version)


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Dreame Smart Vacuum from a config entry."""
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload Dreame Smart Vacuum config entry."""
